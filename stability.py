...
# Damping is considered for the 3rd order
...
filename = 'EE552_Project_data.xlsx'

busdata_excel = pd.read_excel(filename, sheet_name='BusData', engine='openpyxl')
linedata_excel = pd.read_excel(filename, sheet_name='LineData', engine='openpyxl')

# try to improve this maybe numbers or some better idea
bus_df = pd.DataFrame(busdata_excel, columns=['Bus #', 'P MW', 'Q MVAr', 'Type', 'P Gen', 'V Set'])
linedata_df = pd.DataFrame(linedata_excel, columns=['From', 'To', 'R, p.u.', 'X, p.u.', 'B, p.u.'])

bus_np = bus_df.to_numpy()
linedata_np = linedata_df.to_numpy()

# Base Values

Sb = 100

nf = linedata_np[:, 0]  # From
nt = linedata_np[:, 1]  # To
R = linedata_np[:, 2]  # R Line
X = linedata_np[:, 3]  # X Line
B = linedata_np[:, 4]  # B Line
B = B * 1j
Z = R + (X * 1j)  # Z Line
Y = 1 / Z  # Y Line
...
Pset = bus_np[:, 1]  # P from bus data per unit
Qset = bus_np[:, 2]  # Q from bus data per unit
Type = bus_np[:, 3]  # Type from bus data
Vset = bus_np[:, 5]  # Vset from bus data
...
for i in range(len(nf)):
    Ybus[int(nf[i]) - 1][int(nt[i]) - 1] = Ybus[int(nf[i]) - 1][int(nt[i]) - 1] - Y[
        i]  # adding -Y[i] to initial value(zero)
