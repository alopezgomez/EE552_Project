...
import math
import openpyxl
from PowerFlow import test

"Read Excel File"
busdata_excel = pd.read_excel('system_basecase.xlsx', sheet_name='BusData', engine='openpyxl')
linedata_excel = pd.read_excel('system_basecase.xlsx', sheet_name='LineData', engine='openpyxl')
...
linedatadf = pd.DataFrame(linedata_excel, columns=['From', 'To', 'Rtotal, p.u.', 'Xtotal, p.u.', 'Fmax, MVA'])
...
Pset = busnp[:, 1] / Sb  # P from bus data per unit
Qset = busnp[:, 2] / Sb  # Q from bus data per unit
...
for j in range(0, nbus+1):  # nbus+1 = 12+1 = 13 so j max = 12
    for i in range(len(nf)):  # number of rows From
        if int(nf[i]) == j:  # if value From = j
            Ybus[j-1][j-1] = Ybus[j-1][j-1] + Y[i]  # adding Y[i] to initial value (zero)
        elif int(nt[i]) == j:  # if value To = j
            Ybus[j-1][j-1] = Ybus[j-1][j-1] + Y[i]  # adding Y[i] to previous value
...
max_dev = 0.001    # Convergence tolerance, per unit MW and MVAR
...
for iter in range(itermax):
    ...
    for i in range(0, nbus):
        for j in range(0, nbus):
            P[i] += V[i] * V[j] * (G[i][j] * math.cos(thetas[i] - thetas[j]) + B[i][j] * math.sin(thetas[i] - thetas[j]))
            Q[i] += V[i] * V[j] * (G[i][j] * math.sin(thetas[i] - thetas[j]) - B[i][j] * math.cos(thetas[i] - thetas[j]))
    ...
    for i in range(nbus-1):   # range 0 to nbus-1 = 11
        for j in range(nbus-1): # range 0 to nbus-1 = 11
            ...
            if ibus == jbus:  # Diagonal term
                ...
                J[i][j] = V[ibus] * V[jbus] \
                            * (G[ibus][jbus] * math.sin(thetas[ibus] - thetas[jbus]) \
                               - B[ibus][jbus] * math.cos(thetas[ibus] - thetas[jbus]))
    ...
    for i in range(nbus-1, len(Jbus)):  # range  11 to 18
        for j in range(nbus-1):   # range  0 to nbus-1 = 11
            ...
            if ibus == jbus:  # Diagonal term
                ...
                J[i][j] = - V[ibus] * V[jbus] \
                 * (G[ibus][jbus] * math.cos(thetas[ibus] - thetas[jbus]) \
                 +  B[ibus][jbus] * math.sin(thetas[ibus] - thetas[jbus]))
...
for i in range(nbus):
    print('%3d %6.3f %6.2f  %6.1f   %6.1f   %6.1f' % (i+1, V[i], thetas[i]*180/math.pi,
                                                       (P[i]-Pset[i])*Sb,Pset[i]*Sb,P[i]*Sb))
