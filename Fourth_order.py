...
from Third_order import I_0, phi, delta_Ef, Tdo_prime, Id, xd, D, Ef, time_fault
...
Tqo_prime = [0.43, 0.40, 0.38]
xq_prime = [0.38, 0.36, 0.34]
xq = [1.6, 1.4, 1.2]
...
for i in range(0, ng):
    Iq[i] = abs(I_0[i]) * math.cos(-phi[i] + delta_Ef[i])
...
def fault(x, t):
    ...
    d = (-Ed1_prime + Iq[0] * (xq[0] - xq_prime[0])) / Tqo_prime[0]
    e = omega2
    f = (Pm[1] - D[1]*omega1 - (Eq2_prime**2)*G1[1][1] -
    ...
    g = (abs(Ef[1]) - Eq2_prime + Id[1]*(xd[1]-xd_prime[1]))/Tdo_prime[1]
    h = (-Ed2_prime + Iq[1] * (xq[1] - xq_prime[1])) / Tqo_prime[1]
    i = omega3
    j = (Pm[2] - D[2]*omega1 - (Eq3_prime**2)*G1[2][2] -
    ...
    k = (abs(Ef[2]) - Eq3_prime + Id[2] * (xd[2] - xd_prime[2])) / Tdo_prime[2]
    l = (-Ed3_prime + Iq[2] * (xq[2] - xq_prime[2])) / Tqo_prime[2]
...
t_post = np.linspace(time_fault, 1.5, 1000)
